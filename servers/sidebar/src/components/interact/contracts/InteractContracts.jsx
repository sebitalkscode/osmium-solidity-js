...
import { useInteractContracts } from './InteractContracts.logic.js';

export const InteractContracts = ({ wallets, contracts, vscode }) => {
    ...
    <div>
      <div className="dropdown-container">
        ...
        <div className="wallet-container">
          <VSCodeDropdown
            ...
            {...logic.register('wallet', { required: true })}
          >
            {wallets?.map((wallet) => (
        ...
        <div className="contract-container">
          ...
          >
            {contracts?.map((contract) => (
              <VSCodeOption value={contract.id} key={contract.id}>
                {contract.name} - {contract.address}
              </VSCodeOption>
            ))}
          </VSCodeDropdown>
          <VSCodeButton className="add-contract-button" onClick={() => logic.openPanel('tab-contracts')}>
            Edit
          </VSCodeButton>
        </div>
      </div>
      <div className="dropdown-container">
        ...
        <VSCodeDropdown id="dropdown-functions" {...logic.register('function', { required: true })}>
          {logic.functions?.map((func) => (func ? <VSCodeOption value={func} key={func}>{func}</VSCodeOption> : null))}
        </VSCodeDropdown>
      </div>
      <div className="gas-limit-container">
        <VSCodeTextField
          className="gas-limit-textfield"
          {...logic.register('gasLimit', { required: true, valueAsNumber: true })}
        >
          Gas limit
        </VSCodeTextField>
      ...
      <div className="value-container">
        ...
        <div className="value-field-container">
          <VSCodeTextField
            ...
            {...logic.register('value', { required: true, valueAsNumber: true })}
          />
