...
async function runLintForDocument(document) {
  ...
  try {
    const lintDiagnostics = await linter.validateDocument(document);
    finalDiagnostics.push(...(lintDiagnostics || []));
  } catch (_) {}

  try {
    const gasHintsMap = gasEstimation.collectGasHints(documents, getWorkspaceRoot);
    const hints = gasHintsMap.get(document.uri);
    if (hints && hints.length > 0) {
      finalDiagnostics.push(...hints);
    }
  } catch (_) {}

  publishDiagnostics(document.uri, 'linter', finalDiagnostics);
}
...
documents.onDidSave((event) => {
  ...
  // Triggers Analysis
  gasEstimation.analyzeGas();
  slither.runSlitherAnalysis();
});
