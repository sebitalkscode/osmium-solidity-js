...
export const DocsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-zinc-100 font-sans flex">
      <div className="h-screen overflow-y-scroll w-full selection:bg-primary/30">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 py-12 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h1 className="text-4xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">Osmium Documentation</h1>
          <div className="mt-8 bg-surface/50 backdrop-blur-sm border border-zinc-800/80 rounded-xl p-6 shadow-lg">
            ...
            <p className="text-sm font-medium pt-3 border-t border-zinc-800/50 mt-4 text-zinc-300">
              ...
              <Disclosure as="div" key={feature.name} defaultOpen={true}>
                {({ open }) => (
                  <>
                    <dt className="text-lg py-6">
                      <DisclosureButton
                        className={classNames(
                          open ? 'text-primary' : 'text-zinc-300 hover:text-zinc-100',
                          'group w-full flex justify-between items-center text-left bg-surface/40 hover:bg-surface/60 border border-zinc-800/60 rounded-xl p-5 shadow-sm transition-all duration-200',
                        )}
                      >
                    ...
                    <DisclosurePanel as="dd" className="px-5 pt-4 pb-6 mx-2 border-x border-b border-zinc-800/40 rounded-b-xl border-t-0 bg-zinc-900/40 shadow-inner">
